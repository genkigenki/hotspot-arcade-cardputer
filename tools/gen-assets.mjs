...
// Packs are stored raw-DEFLATE'd, not as raw string literals. The ESP32-S3 ROM exports
// tinfl_decompress_mem_to_mem() (see esp32s3.rom.ld), so the inflater costs no flash at
// all, and the packs shrink to ~44% -- worth ~57 KB in an app slot with 11.8 KB spare.
//
// Compression, not a compacter encoding, is deliberate: ha_content.h hands the inflated
// bytes to the SAME parser, so the on-disk grammar stays the contract it is with the
// Flipper host. A denser encoding would have given this host a second dialect to parse,
// which is the one thing ha_content.h's header says must not happen.
//
// Raw deflate (deflateRawSync), not gzip: tinfl reads a bare stream with flags = 0, and
// the 18 bytes of gzip header/trailer per pack are pure loss across ~100 packs.
for (const p of packs) {
  p.z = deflateRawSync(Buffer.from(p.text, 'utf8'), { level: 9 });
  p.raw = Buffer.byteLength(p.text, 'utf8');
}

const totalWeb = files.reduce((n, f) => n + f.bytes.length, 0);
const totalPacks = packs.reduce((n, p) => n + p.raw, 0);
const totalPacksZ = packs.reduce((n, p) => n + p.z.length, 0);
...
//     node tools/gen-assets.mjs
//
// web bundle: ${totalWeb} bytes in ${files.length} file(s).
// packs: ${packs.length} pack(s), ${totalPacks} bytes of text stored in ${totalPacksZ} bytes
// of raw deflate (${Math.round((totalPacksZ / totalPacks) * 100)}%); inflated one game at a time by ha_content.h.
#pragma once
#include <Arduino.h>
#include "ha_proto.h"
...
${packs.map((p, i) => `static const uint8_t HA_PK_${i}[] = {\n${hex(p.z)}};\n`).join('')}
struct HaBakedPack {
    uint8_t game;
    ...
    const uint8_t* z;     // raw-deflate stream of the pack file, verbatim grammar inside
    uint32_t zlen;
    uint32_t rawlen;      // inflated size, so the loader allocates exactly once
};

static const HaBakedPack HA_BAKED_PACKS[] = {
${packs.map((p, i) => `    {${p.gameConst}, "${p.lang}", "${p.fallback}", HA_PK_${i}, ${p.z.length}, ${p.raw}},`).join('\n')}
};
...
console.log(
  `wrote ${out}\n  ${files.length} web file(s), ${totalWeb} bytes\n` +
    `  ${packs.length} pack(s), ${totalPacks} bytes -> ${totalPacksZ} deflated (${Math.round((totalPacksZ / totalPacks) * 100)}%)\n` +
